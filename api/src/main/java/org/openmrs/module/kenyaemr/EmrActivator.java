...
public class EmrActivator implements ModuleActivator {
	...
	private void configureXStreamSecurity() {
		String[] allowedPackages = new String[] {
				...
				"java.util.**",
				"java.lang.**",
				"org.apache.commons.logging.**", // ← ADD: covers SLF4JLocationAwareLog etc.
				"org.apache.commons.**", // ← ADD: broader commons coverage
				"org.slf4j.**", // ← ADD: SLF4J classes
				"ch.qos.logback.**", // ← ADD: Logback classes
				"org.apache.log4j.**" // ← ADD: Log4j classes
		};

		int patchCount = 0;

		// --- Patch 1: XStreamSerializer (serialization.xstream module) ---
		try {
		...
		try {
			...
			if (reportingSerializer != null) {
				// Try standard resolution first (public method or direct field)
				XStream xstream = resolveXStreamInstance(reportingSerializer);

				if (xstream == null) {
					// ReportingSerializer stores XStream inside an anonymous inner class
					// or delegate — use deep field search across the full object graph
		...
		if (patchCount == 0) {
			log.warn("KenyaEMR: No XStream instances were patched. "
					+ "ForbiddenClassException may still occur during report deserialization.");
		} else {
	...
	/**
	 * Resolves the XStream instance from a serializer using two strategies:
	 * 1. A public getXstream() or getXStream() method
	 * 2. Reflection on XStream-typed declared fields across the class hierarchy
	 *
	 * @param serializer the serializer to inspect
	 * @return the XStream instance, or null if not found
	 */
	private XStream resolveXStreamInstance(OpenmrsSerializer serializer) {
		// Strategy 1: public getter method
		...
		// Strategy 2: XStream-typed field anywhere in the class hierarchy
