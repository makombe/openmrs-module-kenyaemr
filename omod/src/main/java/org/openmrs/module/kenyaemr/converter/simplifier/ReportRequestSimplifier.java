...
public class ReportRequestSimplifier extends AbstractSimplifier<ReportRequest> {
	...
	protected SimpleObject simplify(ReportRequest request) {
		// Guard: definition may be null when XStream 1.4.21 (reporting 2.1.0) cannot
		// deserialize the embedded ReportDefinition XML in a stored ReportRequest.
		// This happens because XStream now blocks org.openmrs.module.kenyaemr.**
		// classes via ForbiddenClassException, causing getParameterizable() to
		// return null instead of the actual definition object.
