...
public class GenericUrlFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (path.startsWith(EMR_PREFIX)) {
            ...
            if (qs != null && (qs.contains("kenyaemr.")
                    || qs.contains("%2Fkenyaemr%2F")
                    || qs.contains("/kenyaemr/"))) {
                String cleanQs = rewriteQueryString(qs);
                String redirectUrl = contextPath + path + "?" + cleanQs;
